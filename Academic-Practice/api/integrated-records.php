...
if ($recordId > 0) {
    $stmt = $pdo->prepare("
        SELECT record_id, video_id, mode, title, person_meta, difficulty, duration_label, source_name, country_name, audio_data, audio_mime, created_at
        FROM integrated_response_records
    ...
    listening_json([
        ...
        'record' => [
            ...
            'audioData' => (string)($row['audio_data'] ?? ''),
...
$stmt = $pdo->prepare("
    SELECT record_id, video_id, mode, title, person_meta, difficulty, duration_label, source_name, country_name, created_at
    FROM integrated_response_records
    WHERE user_id = ?
