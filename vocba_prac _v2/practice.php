...
$trainingReturnUrl = vocab_academic_return_url();
$wordbankUrl = vocab_build_url('./wordbank.php');
$practiceSessionBaseUrl = vocab_build_url('./practice-session.php');
$pageBodyClass = 'practicePageBg';
$pageInlineStyles = <<<'CSS'
.practicePageBg{
  background:
    linear-gradient(rgba(255,245,248,.35), rgba(255,245,248,.35)),
    url("./vocabulary/home.png") center top / cover fixed no-repeat;
}
.practicePageBg .page__main{
  position: relative;
}
CSS;
...
?>
        ...
        <section class="vocabEntryHero">
          <button class="modeBackBtn vocabEntryBackBtn" type="button" onclick="window.location.href='<?php echo htmlspecialchars($trainingReturnUrl, ENT_QUOTES, 'UTF-8'); ?>'" aria-label="Go back to training" title="Back">&#8592;</button>
        ...
        <div class="card dailyPlan" style="margin-bottom:18px" id="practiceSelectionBar" aria-label="Word books used for this practice">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap">
            ...
            <a href="<?php echo htmlspecialchars($wordbankUrl, ENT_QUOTES, 'UTF-8'); ?>" class="secondary" style="display:inline-flex;align-items:center;text-decoration:none">Choose word books</a>
          </div>
        ...
        <script>
          ...
          var PRACTICE_SESSION_BASE_URL = <?php echo json_encode($practiceSessionBaseUrl, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>;
          var TITLES = <?php
          ...
          document.getElementById('btnStartSession').addEventListener('click', function() {
            ...
            window.location.href = PRACTICE_SESSION_BASE_URL + (PRACTICE_SESSION_BASE_URL.indexOf('?') === -1 ? '?' : '&') + qs.toString();
          });
