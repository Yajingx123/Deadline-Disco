...
const PostModal = ({ 
  ...
  const parseMarkdown = (text) => {
    ...
    html = html.replace(/!\[audio:(.*?)\]\((.*?)\)/g, (match, fileName, src) => {
      return `<div style="margin: 12px 0; padding: 10px; background: #f4f3ec; border-radius: 8px;"><div style="font-size: 12px; color: #6b6375; margin-bottom: 6px;">🎵 ${fileName}</div><audio controls style="width: 100%; height: 32px;"><source src="${src}" type="audio/mpeg" /><source src="${src}" type="audio/wav" /><source src="${src}" type="audio/ogg" />Your browser does not support the audio element.</audio></div>`;
    });
    html = html.replace(/!\[video:(.*?)\]\((.*?)\)/g, (match, fileName, src) => {
      return `<div style="margin: 12px 0; padding: 10px; background: #f4f3ec; border-radius: 8px;"><div style="font-size: 12px; color: #6b6375; margin-bottom: 6px;">🎬 ${fileName}</div><video controls style="width: 100%; max-height: 320px; border-radius: 8px;"><source src="${src}" type="video/mp4" /><source src="${src}" type="video/webm" />Your browser does not support the video element.</video></div>`;
    });
