...
$requestHost = (string)($_SERVER['HTTP_HOST'] ?? '127.0.0.1:8001');
$scriptName = str_replace('\\', '/', (string)($_SERVER['SCRIPT_NAME'] ?? ''));
$appPrefix = rtrim(str_replace('\\', '/', dirname(dirname($scriptName))), '/.');
$detectedAppUrl = $requestScheme . '://' . $requestHost . ($appPrefix !== '' ? $appPrefix : '');
$requestHostName = strtolower((string)preg_replace('/:\d+$/', '', $requestHost));
$isLocalRequestHost = in_array($requestHostName, ['127.0.0.1', 'localhost'], true);
$configuredAppUrl = trim((string)(getenv('APP_URL') ?: ''));
$appBaseUrl = rtrim(($isLocalRequestHost || $configuredAppUrl === '') ? $detectedAppUrl : $configuredAppUrl, '/');
...
define('APP_VOCAB_ROOT_PATH', '/vocba_prac%20_v2/');
...
$uiMode = strtolower(trim((string)($_GET['ui'] ?? '')));
if ($uiMode === 'godot') {
  setcookie('ui_mode', 'godot', [
...
function vocab_is_godot_ui(): bool {
  $ui = strtolower(trim((string)($_GET['ui'] ?? '')));
  if ($ui === 'godot') {
    return true;
  }
  $cookie = strtolower(trim((string)($_COOKIE['ui_mode'] ?? '')));
  return $cookie === 'godot';
}

function vocab_is_local_host(): bool {
  $host = strtolower((string)($_SERVER['HTTP_HOST'] ?? $_SERVER['SERVER_NAME'] ?? ''));
  $host = preg_replace('/:\d+$/', '', $host);
  return in_array($host, ['127.0.0.1', 'localhost'], true);
}

function vocab_build_url(string $path, array $params = []): string {
  if (vocab_is_godot_ui() && !array_key_exists('ui', $params)) {
    $params['ui'] = 'godot';
  }
...
function vocab_academic_return_url(): string {
  if (!vocab_is_godot_ui()) {
    return '../Academic-Practice/training.html';
  }
  if (vocab_is_local_host()) {
    $scheme = (!empty($_SERVER['HTTPS']) && strtolower((string)$_SERVER['HTTPS']) !== 'off') ? 'https' : 'http';
    $host = strtolower((string)($_SERVER['HTTP_HOST'] ?? $_SERVER['SERVER_NAME'] ?? '127.0.0.1'));
    $host = preg_replace('/:\d+$/', '', $host);
    return $scheme . '://' . $host . ':5500/index.html?ui=godot&scene=academic';
  }
  return ACADBEAT_MAIN_ORIGIN . '/gameUI_src/Release/index.html?ui=godot&scene=academic';
}
...
function vocab_require_auth(): void {
  ...
  $currentUri = (string)($_SERVER['REQUEST_URI'] ?? APP_VOCAB_ROOT_PATH);
  $target = APP_HOME_URL . '?login=1&from=' . rawurlencode($currentUri);
  if (!headers_sent()) {
