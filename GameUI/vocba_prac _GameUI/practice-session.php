...
if (!$bookSlugs) {
  header('Location: ' . vocab_build_url('./wordbank.php'), true, 302);
  exit;
}
...
// 顶栏需要显示 mode
$pageBodyClass = 'practiceSessionPageBg';
$practiceUrl = vocab_build_url('./practice.php');
$pageInlineStyles = <<<'CSS'
.practiceSessionPageBg{
  background:
    linear-gradient(rgba(248,245,255,.35), rgba(248,245,255,.35)),
    url("./vocabulary/bei.png") center top / cover fixed no-repeat;
}
.practiceSessionPageBg .page__main{
  position: relative;
}
CSS;
...
?>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:14px">
          <div style="color:rgba(17,24,39,.88);font-size:13px">
            Session: <strong><?php echo htmlspecialchars($mode === 1 ? '1 minute' : ($mode === 5 ? '3-5 minutes' : '10 minutes')); ?></strong>
          </div>
          <a href="<?php echo htmlspecialchars($practiceUrl, ENT_QUOTES, 'UTF-8'); ?>" class="sessionExitBtn">Exit</a>
        </div>
          ...
          (function() {
            ...
            function renderEnd() {
              ...
              contentEl.innerHTML =
                ...
                '<a href="<?php echo htmlspecialchars($practiceUrl, ENT_QUOTES, 'UTF-8'); ?>" class="primary" style="display:inline-flex;align-items:center;text-decoration:none;padding:12px 18px">Back to Practice</a>' +
                '</div>';
            }
