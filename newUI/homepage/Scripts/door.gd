...
func _open_forum_with_prompt() -> void:
	var prompt_text := "Enter Forum now?\\n(Press OK to continue)"
	if OS.has_feature("web"):
		var should_enter = JavaScriptBridge.eval("window.confirm('" + prompt_text + "');")
		if should_enter:
			_navigate_to_url(MAIN_FORUM_URL)
	else:
		# Desktop preview fallback: no browser modal, go directly.
		_navigate_to_url(MAIN_FORUM_URL)
