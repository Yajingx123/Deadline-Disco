...
CREATE TABLE IF NOT EXISTS peer_video_rooms (
    room_id BIGINT PRIMARY KEY AUTO_INCREMENT,
    room_public_id VARCHAR(40) NOT NULL,
    zego_room_id VARCHAR(128) NOT NULL,
    host_user_id BIGINT NOT NULL,
    topic_key VARCHAR(50) NULL,
    topic_label VARCHAR(160) NOT NULL,
    visibility VARCHAR(20) NOT NULL,
    status VARCHAR(20) NOT NULL DEFAULT 'open',
    active_member_count INT NOT NULL DEFAULT 1,
    peak_member_count INT NOT NULL DEFAULT 1,
    expires_at DATETIME NOT NULL,
    ended_at DATETIME NULL,
    ended_reason VARCHAR(30) NULL,
    last_member_joined_at DATETIME NULL,
    metadata_json JSON NULL,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    CONSTRAINT uq_peer_video_rooms_public UNIQUE (room_public_id),
    CONSTRAINT uq_peer_video_rooms_zego UNIQUE (zego_room_id),

    CONSTRAINT fk_peer_video_rooms_host
        FOREIGN KEY (host_user_id)
        REFERENCES users(user_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE,

    CONSTRAINT chk_peer_video_rooms_visibility
        CHECK (visibility IN ('public', 'private')),

    CONSTRAINT chk_peer_video_rooms_status
        CHECK (status IN ('open', 'ended', 'cancelled', 'expired')),

    CONSTRAINT chk_peer_video_rooms_counts
        CHECK (active_member_count >= 0 AND peak_member_count >= 1),

    CONSTRAINT chk_peer_video_rooms_ended_reason
        CHECK (ended_reason IS NULL OR ended_reason IN ('host_ended', 'expired', 'single_member_timeout', 'host_left', 'system'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS peer_video_room_invites (
    ...
    CONSTRAINT uq_peer_video_room_invites_token UNIQUE (invite_token),

    CONSTRAINT fk_peer_video_room_invites_room
        FOREIGN KEY (room_id)
        REFERENCES peer_video_rooms(room_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE,

    CONSTRAINT fk_peer_video_room_invites_creator
        FOREIGN KEY (created_by_user_id)
        REFERENCES users(user_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE,

    CONSTRAINT fk_peer_video_room_invites_target
        FOREIGN KEY (target_user_id)
        REFERENCES users(user_id)
        ON DELETE SET NULL
        ON UPDATE CASCADE,

    CONSTRAINT fk_peer_video_room_invites_consumed_by
        FOREIGN KEY (consumed_by_user_id)
        REFERENCES users(user_id)
        ON DELETE SET NULL
        ON UPDATE CASCADE,

    CONSTRAINT chk_peer_video_room_invites_status
        CHECK (status IN ('active', 'consumed', 'revoked', 'expired'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS peer_video_room_members (
    room_member_id BIGINT PRIMARY KEY AUTO_INCREMENT,
    room_id BIGINT NOT NULL,
    user_id BIGINT NOT NULL,
    ...
    last_entered_room_at DATETIME NULL,
    last_left_room_at DATETIME NULL,
    removed_by_user_id BIGINT NULL,
    removed_at DATETIME NULL,
    remove_reason VARCHAR(30) NULL,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    CONSTRAINT uq_peer_video_room_members_unique UNIQUE (room_id, user_id),

    CONSTRAINT fk_peer_video_room_members_room
        FOREIGN KEY (room_id)
        REFERENCES peer_video_rooms(room_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE,

    CONSTRAINT fk_peer_video_room_members_user
        FOREIGN KEY (user_id)
        REFERENCES users(user_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE,

    CONSTRAINT fk_peer_video_room_members_invite
        FOREIGN KEY (invite_id)
        REFERENCES peer_video_room_invites(invite_id)
        ON DELETE SET NULL
        ON UPDATE CASCADE,

    CONSTRAINT fk_peer_video_room_members_removed_by
        FOREIGN KEY (removed_by_user_id)
        REFERENCES users(user_id)
        ON DELETE SET NULL
        ON UPDATE CASCADE,

    CONSTRAINT chk_peer_video_room_members_role
        CHECK (role IN ('host', 'participant')),

    CONSTRAINT chk_peer_video_room_members_membership_status
        CHECK (membership_status IN ('active', 'left', 'removed')),

    CONSTRAINT chk_peer_video_room_members_presence_status
        CHECK (presence_status IN ('offline', 'joining', 'in_room')),

    CONSTRAINT chk_peer_video_room_members_joined_via
        CHECK (joined_via IN ('host_create', 'public_list', 'invite_link', 'room_link_reentry')),

    CONSTRAINT chk_peer_video_room_members_remove_reason
        CHECK (remove_reason IS NULL OR remove_reason IN ('kicked_by_host', 'room_ended', 'room_expired', 'host_left', 'system'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS peer_video_room_events (
    event_id BIGINT PRIMARY KEY AUTO_INCREMENT,
    room_id BIGINT NOT NULL,
    actor_user_id BIGINT NULL,
    target_user_id BIGINT NULL,
    ...
    CONSTRAINT fk_peer_video_room_events_room
        FOREIGN KEY (room_id)
        REFERENCES peer_video_rooms(room_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE,

    CONSTRAINT fk_peer_video_room_events_actor
        FOREIGN KEY (actor_user_id)
        REFERENCES users(user_id)
        ON DELETE SET NULL
        ON UPDATE CASCADE,

    CONSTRAINT fk_peer_video_room_events_target
        FOREIGN KEY (target_user_id)
        REFERENCES users(user_id)
        ON DELETE SET NULL
        ON UPDATE CASCADE,

    CONSTRAINT chk_peer_video_room_events_type
        CHECK (event_type IN (
            'room_created',
            'member_joined',
            'member_reentered',
...
CREATE INDEX idx_peer_video_rooms_host_status
    ON peer_video_rooms(host_user_id, status, expires_at);

CREATE INDEX idx_peer_video_rooms_visibility_status
    ON peer_video_rooms(visibility, status, expires_at);

CREATE INDEX idx_peer_video_room_invites_room_status
    ON peer_video_room_invites(room_id, status, expires_at);

CREATE INDEX idx_peer_video_room_invites_target
    ON peer_video_room_invites(target_user_id, status, expires_at);

CREATE INDEX idx_peer_video_room_members_user_status
    ON peer_video_room_members(user_id, membership_status, presence_status);

CREATE INDEX idx_peer_video_room_members_room_status
    ON peer_video_room_members(room_id, membership_status, presence_status);

CREATE INDEX idx_peer_video_room_events_room_created
    ON peer_video_room_events(room_id, created_at);

CREATE INDEX idx_peer_video_room_events_actor_created
    ON peer_video_room_events(actor_user_id, created_at);
